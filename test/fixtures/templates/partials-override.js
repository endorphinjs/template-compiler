import { elemWithText, createComponent, setAttribute, mountComponent, updateComponent, unmountComponent, assign, subscribeStore, addDisposeCallback, markSlotUpdate, elem, insert, text, updateText, finalizeAttributes, createInjector } from "@endorphinjs/endorphin";
...
function $$partialMyItem0(host, injector, scope) {
	...
	setAttribute(injector0, "value", host.store.data.pos);
	scope.$_text0 = span0.appendChild(text(host.store.data.item));
...
function $$partialMyItem0Update(host, injector, scope) {
	...
	setAttribute(injector0, "value", host.store.data.pos);
	updateText(scope.$_text0, host.store.data.item);
	finalizeAttributes(injector0);
}
...
export default function $$template0(host, scope) {
	...
	setAttribute(injector0, "items", host.store.data.items1);
	...
	setAttribute(injector1, "items", host.store.data.items2);
...
function $$template0Update(host, scope) {
	...
	s__innerComponent0 |= setAttribute(injector0, "items", host.store.data.items1);
	markSlotUpdate(scope.$_innerComponent0, "", s__innerComponent0);
	updateComponent(scope.$_innerComponent0);
	let s__innerComponent1 = 0;
	s__innerComponent1 |= setAttribute(injector1, "items", host.store.data.items2);
